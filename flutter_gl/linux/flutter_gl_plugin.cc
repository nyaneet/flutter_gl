#include "include/flutter_gl/flutter_gl_plugin.h"
...
G_DEFINE_TYPE(FlutterGlPlugin, flutter_gl_plugin, g_object_get_type())
...
static void flutter_gl_plugin_handle_method_call(
	FlutterGlPlugin *self,
	FlMethodCall *method_call)
{



    //printf(".... flutter_gl_plugin_handle_method_call\n");
...
static void flutter_gl_plugin_dispose(GObject *object)
{
    printf(".... displose\n");
	G_OBJECT_CLASS(flutter_gl_plugin_parent_class)->dispose(object);
}

static void flutter_gl_plugin_class_init(FlutterGlPluginClass *klass)
{
    printf(".... class init\n");

    G_OBJECT_CLASS(klass)->dispose = flutter_gl_plugin_dispose;
}

static void flutter_gl_plugin_init(FlutterGlPlugin *self) {}

static void method_call_cb(FlMethodChannel *channel, FlMethodCall *method_call,
						   gpointer user_data)
{
    //printf(".... method_call_cb\n");
	FlutterGlPlugin *plugin = FLUTTER_GL_PLUGIN(user_data);
    //mylog("method call in");
	flutter_gl_plugin_handle_method_call(plugin, method_call);
    //mylog("method call out");
}

void flutter_gl_plugin_register_with_registrar(FlPluginRegistrar *registrar)
{
    printf(".... register with register\n");
   // mylog("with register");
	FlutterGlPlugin *plugin = FLUTTER_GL_PLUGIN(
		g_object_new(flutter_gl_plugin_get_type(), nullptr));
