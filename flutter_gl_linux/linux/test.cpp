...
#include "include/flutter_gl_linux/flutter_gl_linux_plugin.h"
...
GLuint createRenderTargetTexture(FlutterGlLinuxPlugin *self, uint16_t const width, uint16_t const height)
{
    GLuint frameBuffer = 0;
    glGenFramebuffers(1, &frameBuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer);
...
void mainFBO4(FlutterGlLinuxPlugin *self, int width, int height)
{
    ...
    std::thread gl_thread([](FlutterGlLinuxPlugin *self, Shader *shader, int width, int height)
                          {
...
void mainFBO5(FlutterGlLinuxPlugin *self, int width, int height)
{
    ...
    std::thread gl_thread([](FlutterGlLinuxPlugin *self, int width, int height)
                          {
...
void mainFBO(FlutterGlLinuxPlugin *self, int width, int height)
{
    ...
    glGenFramebuffers(1, &framebuffer);
    eglPrintError("glGenFramebuffers()");
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    eglPrintError("glBindFramebuffer()");
    ...
    eglPrintError("glGenRenderbuffers()");
    glBindRenderbuffer(GL_RENDERBUFFER, rbo);
    eglPrintError("glBindRenderbuffer()");
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width, height);
    eglPrintError("glRenderbufferStorage()");
    glBindRenderbuffer(GL_RENDERBUFFER, 0);
    eglPrintError("glBindRenderbuffer()");

    // attach the renderbuffer object to the depth and stencil attachment of the framebuffer
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rbo);
    eglPrintError("glFramebufferRenderbuffer()");
...
void testRandomTexture(FlutterGlLinuxPlugin *self, int width, int height)
{
